...
class Scraper {
    ...
    async fetchInternalLinks(internalLinks) {
        const result = await Promise.all(internalLinks.map((link) => {
    ...
    findInternalLinks() {
        ...
        [...links].map((link) => Scraper.internalLinks.add(link.getAttribute("href")));
    }
    findEmails() {
    ...
    validateEmails(emails) {
        const isEmails = emails.filter(email => emailValidator.validate(email)).filter(email => {
